...
class PrismaticForConditionalGeneration(PrismaticPreTrainedModel):
    def __init__(self, config: PrismaticConfig) -> None:
        ...
        # Relax TIMM version check for compatibility
        if timm.__version__ not in {"0.9.10", "0.9.11", "0.9.12", "0.9.16", "1.0.26"}:
            logger.warning(
                f"Expected TIMM version in [0.9.10, 0.9.16] but got {timm.__version__}; "
                f"proceeding anyway. If you encounter issues, please use a supported version."
            )
    ...
    def tie_weights(self, recompute_mapping: bool = False) -> None:
